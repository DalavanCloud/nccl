...
if [ "$all" == "1" ]; then
  for dtype in float double half int8 int32 int64 uint8 uint32 uint64 ; do
    for otype in sum max min prod ; do
      ...
      $srun_cmd test/perf/${op}_perf -t $ngpus -d $dtype -o $otype -b 64 -e 4194304 -f 256 $extra -w 10 -n 10 | tee $result.pow2.out
      $srun_cmd test/perf/${op}_perf -t $ngpus -d $dtype -o $otype -b 63 -e 4357647 -f 263 $extra -w 10 -n 10 | tee $result.npow2.out
    done
  done
  return 0
fi
...
if [ "$mpi" == "0" ]; then
  ...
  if [ "$reorder" == "0" ]; then
    $srun_cmd test/perf/${op}_perf -t $ngpus -b 40000 -e 1960000 -i 40000 $extra -w 20 -n 20 | tee $result.out
    $srun_cmd test/perf/${op}_perf -t $ngpus -p 1 -b 2000000 -e 38000000 -i 2000000 $extra -w 20 -n 5 | tee -a $result.out
  fi
  eval $SET_VISIBLE $srun_cmd test/perf/${op}_perf -g $ngpus -b 40000000 -e 400000000 -i 40000000 $extra -w 5 -n 1 | tee -a $result.out
else
  echo "Running test/perf/${op}_perf on $ngpus GPUs [MPI] ..."
  if [ "$reorder" == "0" ]; then
    $salloc_cmd mpirun -x NCCL_DEBUG -np $ngpus test/perf/${op}_perf -b 40000 -e 1960000 -i 40000 $extra -w 20 -n 20 | tee $result.out
    $salloc_cmd mpirun -x NCCL_DEBUG -np $ngpus test/perf/${op}_perf -b 2000000 -e 38000000 -i 2000000 $extra -w 20 -n 5 | tee -a $result.out
  fi
  $salloc_cmd mpirun -x NCCL_DEBUG $SET_VISIBLE -np $ngpus test/perf/${op}_perf -b 40000000 -e 400000000 -i 40000000 $extra -w 5 -n 1 | tee -a $result.out
fi
}
