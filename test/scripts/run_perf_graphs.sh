...
if [ "$all" == "1" ]; then
  ...
  for dtype in float double half int8 int32 int64 uint8 uint32 uint64 ; do
    for otype in sum max min prod ; do
      ...
      $srun_cmd test/perf/${op}_perf -t $ngpus -d $dtype -o $otype -b 64 -e 4194304 -f 256 $extra -w 10 -n 10 2>&1 | tee $path/pow2/$result.out
      $srun_cmd test/perf/${op}_perf -t $ngpus -d $dtype -o $otype -b 63 -e 4357647 -f 263 $extra -w 10 -n 10 2>&1 | tee $path/npow2/$result.out
    done
  done
  return 0
fi
...
if [ "$mpi" == "0" ]; then
  ...
  eval $SET_VISIBLE $srun_cmd test/perf/${op}_perf -g $ngpus -b 40000000 -e 400000000 -i 40000000 $extra -w 5 -n 1 2>&1 | tee -a $result.out
else
