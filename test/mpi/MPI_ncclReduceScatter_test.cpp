...
TYPED_TEST(mpi_test, ncclReduceScatter_basic) {
    for (auto op : this->RedOps) {
        ...
        MNCCL_ASSERT(ncclReduceScatter(
            ...
            this->count1 / TEST_ENV::mpi_size, this->ncclDataType, op,
        ...
        // mpi result
        std::vector<int> recvcounts(TEST_ENV::mpi_size,
                                    this->count1 / TEST_ENV::mpi_size);
        MPI_Reduce_scatter(this->buf_send_h.data(), this->buf_recv_mpi.data(),
                           recvcounts.data(), this->mpiDataType,
                           this->MpiOps.at(op), MPI_COMM_WORLD);
        EXPECT_NO_FATAL_FAILURE(
            this->Verify(this->count1 / TEST_ENV::mpi_size));
    }
}
