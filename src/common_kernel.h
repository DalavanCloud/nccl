...
template <int type>
static void LaunchProxy(int substeps, int nsteps, struct ncclRing* ring) {
...
static ncclResult_t LaunchProxies(int substeps, int subchunks, int nsteps_per_round, int nblocks_per_round, int size, struct ncclComm* comm) {
  for (int r=0; r<comm->nRings; r++) {
    int nrounds = DIVUP(size,nblocks_per_round * (comm->rings[r].buffSize/subchunks));
...
template <int type>
static ncclResult_t WaitProxy(struct ncclRing* ring) {
  struct ncclConnector* connector = (type == 0) ? &ring->recv : &ring->send;
  threadFunc_t proxyfunc = (threadFunc_t) ((type == 0) ? connector->transport->recv.proxy : connector->transport->send.proxy);
  if (proxyfunc) {
    ncclResult_t ret;
...
static ncclResult_t WaitProxies(struct ncclComm* comm) {
  for (int r=0; r<comm->nRings; r++) {
