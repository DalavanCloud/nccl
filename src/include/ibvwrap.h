...
extern const char * (*ibv_internal_get_device_name)(struct ibv_device *device);
extern struct ibv_context* (*ibv_internal_open_device)(struct ibv_device* device);
extern int (*ibv_internal_close_device)(struct ibv_context *context);
extern int (*ibv_internal_get_async_event)(struct ibv_context *context, struct ibv_async_event *event);
extern void (*ibv_internal_ack_async_event)(struct ibv_async_event *event);
extern int (*ibv_internal_query_device)(struct ibv_context *context, struct ibv_device_attr *device_attr);
extern int (*ibv_internal_query_port)(struct ibv_context *context, uint8_t port_num, struct ibv_port_attr *port_attr);
extern struct ibv_pd * (*ibv_internal_alloc_pd)(struct ibv_context *context);
extern struct ibv_mr * (*ibv_internal_reg_mr)(struct ibv_pd *pd, void *addr, size_t length, int access);
extern int (*ibv_internal_dereg_mr)(struct ibv_mr *mr);
extern struct ibv_comp_channel * (*ibv_internal_create_comp_channel)(struct ibv_context *context);
extern struct ibv_cq * (*ibv_internal_create_cq)(struct ibv_context *context, int cqe, void *cq_context, struct ibv_comp_channel *channel, int comp_vector);
extern int (*ibv_internal_poll_cq)(struct ibv_cq *cq, int num_entries, struct ibv_wc *wc);
extern struct ibv_qp * (*ibv_internal_create_qp)(struct ibv_pd *pd, struct ibv_qp_init_attr *qp_init_attr);
extern int (*ibv_internal_modify_qp)(struct ibv_qp *qp, struct ibv_qp_attr *attr, int attr_mask);
extern int (*ibv_internal_post_send)(struct ibv_qp *qp, struct ibv_send_wr *wr, struct ibv_send_wr **bad_wr);
extern int (*ibv_internal_post_recv)(struct ibv_qp *qp, struct ibv_recv_wr *wr, struct ibv_recv_wr **bad_wr);
extern const char * (*ibv_internal_event_type_str)(enum ibv_event_type event);

#define USE_DYNLD_SYMS 1
/*Declarations*/
...
#else
   INFO("Dynld ibv_get_device_list");   
  return ibv_internal_get_device_list(num_devices); 
#endif
}
static void wrap_ibv_free_device_list(struct ibv_device **list) { 
#ifndef USE_DYNLD_SYMS
   INFO("Static ibv_free_device_list");   
  return ibv_free_device_list(list); 
#else
   INFO("Dynld ibv_free_device_list");   
  return ibv_internal_free_device_list(list); 
#endif
}
static const char *wrap_ibv_get_device_name(struct ibv_device *device) { 
#ifndef USE_DYNLD_SYMS
   INFO("Static ibv_get_device_name");   
  return ibv_get_device_name(device); 
#else
   INFO("Dynld ibv_get_device_name");   
  return ibv_internal_get_device_name(device); 
#endif
}
static struct ibv_context *wrap_ibv_open_device(struct ibv_device *device) { 
#ifndef USE_DYNLD_SYMS
   INFO("Static ibv_open_device");   
   return ibv_open_device(device);
#else
   INFO("Dynld ibv_open_device"); 
   return ibv_internal_open_device(device);
#endif
}
static int wrap_ibv_close_device(struct ibv_context *context) { 
#ifndef USE_DYNLD_SYMS
   INFO("Static ibv_close_device");   
  return ibv_close_device(context); 
#else
   INFO("Dynld ibv_close_device"); 
  return ibv_internal_close_device(context); 
#endif
}
static int wrap_ibv_get_async_event(struct ibv_context *context, struct ibv_async_event *event) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_get_async_event");
  return ibv_get_async_event(context, event); 
#else
  INFO("Dynld ibv_get_async_event");
  return ibv_internal_get_async_event(context, event); 
#endif
}
static void wrap_ibv_ack_async_event(struct ibv_async_event *event) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_ack_async_event");
  return ibv_ack_async_event(event); 
#else
  INFO("Dynld ibv_ack_async_event");
  return ibv_internal_ack_async_event(event); 
#endif
}
static int wrap_ibv_query_device(struct ibv_context *context, struct ibv_device_attr *device_attr) {
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_ack_async_event");
   return ibv_query_device(context, device_attr); 
#else
  INFO("Dynld ibv_query_device");
   return ibv_internal_query_device(context, device_attr); 
#endif
}
static int wrap_ibv_query_port(struct ibv_context *context, uint8_t port_num, struct ibv_port_attr *port_attr) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_ack_async_event");
  return ibv_query_port(context, port_num, port_attr); 
#else
  INFO("Dynld ibv_query_port");
  return ibv_internal_query_port(context, port_num, port_attr); 
#endif
}
static struct ibv_pd *wrap_ibv_alloc_pd(struct ibv_context *context) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_alloc_pd");
  return ibv_alloc_pd(context); 
#else
  INFO("Dynld ibv_alloc_pd");
  return ibv_internal_alloc_pd(context); 
#endif
}
static struct ibv_mr *wrap_ibv_reg_mr(struct ibv_pd *pd, void *addr, size_t length, int access) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_reg_mr");
  return ibv_reg_mr(pd, addr, length, access); 
#else
  INFO("Dynld ibv_reg_mr");
  return ibv_internal_reg_mr(pd, addr, length, access); 
#endif
}
static int wrap_ibv_dereg_mr(struct ibv_mr *mr) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_dereg_mr");
  return ibv_dereg_mr(mr); 
#else
  INFO("Dynld ibv_dereg_mr");
  return ibv_internal_dereg_mr(mr); 
#endif
}
static struct ibv_comp_channel *wrap_ibv_create_comp_channel(struct ibv_context *context) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_create_comp_channel");
  return ibv_create_comp_channel(context); 
#else
  INFO("Dynld ibv_create_comp_channel");
  return ibv_internal_create_comp_channel(context); 
#endif
}
static struct ibv_cq *wrap_ibv_create_cq(struct ibv_context *context, int cqe, void *cq_context, struct ibv_comp_channel *channel, int comp_vector) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_create_cq");
...
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_create_qp");
  return ibv_create_qp(pd, qp_init_attr);
#else
  INFO("Dynld ibv_create_qp");
  return ibv_internal_create_qp(pd, qp_init_attr); 
#endif
}
static int wrap_ibv_modify_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr, int attr_mask) { 
#ifndef USE_DYNLD_SYMS
  INFO("Static ibv_modify_qp");
